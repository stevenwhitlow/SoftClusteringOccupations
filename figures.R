...
###
# Plots the predicted wage change based solely on changes in group membership against actual wage changes
...
###
# Plots the empirical distribution function of the highest degree of membership for each occupation
classifications %>% 
...
###
# Sankey diagram pre and post displacement

# Group by current and former primary groupings
displaced %>%
...
# Generate changes in wage weighted by survey weight.
...
#####
## WARRNING: ggtern needs to be unloaded before using ggplot again for other figures
library(ggtern)
...
classifications_simplex <- classifications_simplex %>%
    inner_join(svy_census %>% select(occnumber, perwt, lnhrwage) %>%
    group_by(occnumber) %>%
    summarise(lnhrwage = survey_mean(lnhrwage)))

# Generate cross-sectional mean wages
census_mean_wage <- svy_census %>%
    select(occnumber, perwt, lnhrwage) %>%
    summarise(lnhrwage = survey_mean(lnhrwage))

#G enerate relative wage
classifications_simplex <- classifications_simplex %>%
...
# Calculate the percentage of workers in 2000 who have a significant degree of mixing over pure types 
# i.e. where degree of membership of primary type is less than 95%
classifications_simplex %>%
    filter(freq1<0.95 & freq2<0.95 & freq3<0.95 & freq4<0.95 & freq5 <0.95 & freq6<0.95 & freq7<0.95) %>%
    summarise(sum(percent_workforce))

# Combine into three types
classifications_simplex %>% 
