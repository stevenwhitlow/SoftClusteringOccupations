...
Skills <- subset(Skills, `Scale ID`=="IM")
...
#tasks <- tasks %>% mutate(`Data Value` = replace(`Data Value`, which(`Data Value`>2.5 & `Data Value`<=3.5), 1))
#tasks <- tasks %>% mutate(`Data Value` = replace(`Data Value`, which(`Data Value`>3.5), 2))
...
for (j in 1:J) {
  theta[j, , ] <- gtools::rdirichlet(K, rep(0.99, Vj[j]))
}
...
occupations$occnumber = paste("Ind",1:nrow(occupations))

classifications <- inner_join(occupations, lambda)
classifications$puregroup <- substr(classifications$puregroup, 7,7)
